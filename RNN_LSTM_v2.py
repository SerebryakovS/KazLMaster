...
def TrainLSTMModel(VocabularySize, DataSets):
    ...
    # # Load training, validation, and test datasets from the default path
    ...
    # Define a uniform random initializer with the specified initialization scale from the config
    initializer = tf.compat.v1.random_uniform_initializer(-config.init_scale, config.init_scale)
    # Define the training model within a TensorFlow variable scope
    ...
    # Define the validation model with the same variable scope as the training model
    with tf.compat.v1.variable_scope('Model', reuse=True, initializer=initializer):
        m_valid = MyModel(config=config, is_train=False)
    # Define the test model with the same variable scope as the training model
    with tf.compat.v1.variable_scope('Model', reuse=True, initializer=initializer):
        m_test = MyModel(config=config, is_train=False)
    # Create a saver object to save and restore TensorFlow model variables
    ...
    # Start a TensorFlow session with GPU configuration
    with tf.compat.v1.Session(config=GPUconfig) as sess:
        # Run the variables initializer
        sess.run(init)
        # Set initial values for previous and best validation set perplexity
        prev_valid_ppl = float('inf')
        best_valid_ppl = float('inf')
        # Train the model for a number of epochs
        for epoch in range(num_epochs):
            # Run one training epoch and get the perplexity
            train_ppl = run_epoch(
